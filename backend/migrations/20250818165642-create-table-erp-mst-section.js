...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.createTable('erp_mst_sections', {
      mst_section_id: { type: Sequelize.INTEGER, autoIncrement: true, primaryKey: true },
      ...
      capacity: { type: Sequelize.INTEGER, allowNull: true,defaultValue: 0 },  
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    if (queryInterface.sequelize.getDialect() === 'postgres') {
      await queryInterface.sequelize.query('DROP TYPE IF EXISTS "enum_erp_mst_sections_is_active";');
    }
  }
};
