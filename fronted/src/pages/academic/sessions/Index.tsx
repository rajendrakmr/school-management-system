...
import { useGetSessionsQuery } from "@/store/slice/academics/session";

const Index: React.FC = () => {
  const [filter, setFilter] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const dispatch = useDispatch(); 
  const title ="Academic Session"
  useEffect(() => {
    dispatch(setBreadcrumbs([title]));
  }, [dispatch]);

  const { data: referenceRecord} = useGetColumnsQuery({ type: "sessions" },{ refetchOnMountOrArgChange: true } );
  ...
  const { data: dataRecords, isFetching, refetch } = useGetSessionsQuery(
