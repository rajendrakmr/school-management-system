...
import { useSaveSectionMutation } from "@/store/slice/academics/sections";
...
const AddEditForm: React.FC<AddEditFormProps> = React.memo(
    ({ open, onClose, initialData, isEdit, onSuccess, pagetitle }) => {
        ...
        const [saveSection, { isLoading }] = useSaveSectionMutation();
        useEffect(() => {
        ...
        const handleFormSubmit = useCallback(async () => {
            ...
            try {
                const response = await saveSection(formData).unwrap();
                ...
                if (err?.data?.errors) {
                    console.log('err?.data?.errorserr?.data?.errors', err?.data?.errors)
        ...
        return (
            ...
            > <div className="row">
               ...
               <div className="col-md-12"></div>
                    ...
                    />
                    <InputFormField
                        label={fieldConfigs.capacity.label}
                        name="capacity"
                        inputValue={formData.capacity}
                        error={errors.capacity}
                        required={fieldConfigs.capacity.required}
                        onChange={handleChange}
                        col="col-md-3"
                        type="num"
                    />
