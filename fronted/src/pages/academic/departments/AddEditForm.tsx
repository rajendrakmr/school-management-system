...
import { useSaveDepartmentMutation } from "@/store/slice/academics/department";

interface FormRecordItem {
    mst_department_id: string;
...
const AddEditForm: React.FC<AddEditFormProps> = React.memo(
    ({ open, onClose, initialData, isEdit, onSuccess }) => { 
        ...
        const fieldConfigs: Record<keyof FormRecordItem, { label: string; required: boolean; minLength?: number; maxLength?: number }> = {
            mst_department_id: { required: false, label: "ID" },
        ...
        const handleFormSubmit = useCallback(async () => {
            ...
            try {
                const response = await saveDepartment(formData).unwrap();
        ...
        }, [formData, onSuccess, resetForm, onClose, saveDepartment]);
