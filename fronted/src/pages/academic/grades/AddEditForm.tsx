...
import { useSaveGradeMutation } from "@/store/slice/academics/grades";
...
const AddEditForm: React.FC<AddEditFormProps> = React.memo(
    ({ open, onClose, initialData, isEdit, onSuccess }) => { 
        ...
        const fieldConfigs: Record<keyof FormRecordItem, { label: string; required: boolean; minLength?: number; maxLength?: number }> = {
            mst_grage_id: { required: false, label: "ID" },
            trn_school_id: { required: !usersInfo?.trn_school_id, label: "Branch" },
            name: { required: true, minLength: 2, maxLength: 50, label: "Name" },
            max_percentage: { required: true, label: "Maximu Percentage" },
            min_percentage: { required: true, label: "Minimum Percentage" },
        ...
        const handleFormSubmit = useCallback(async () => {
            ...
            try {
                const response = await saveGrade(formData).unwrap();
        ...
        }, [formData, onSuccess, resetForm, onClose, saveGrade]);
        ...
        return (
            ...
            >
                ...
                <div className="col-md-"></div>
                  <InputFormField
                    label={fieldConfigs.max_percentage.label}
                    name="max_percentage"
                    inputValue={formData.max_percentage}
                    error={errors.max_percentage}
                    required={fieldConfigs.max_percentage.required}
                    onChange={handleChange}
                    col="col-md-3"
                    type="num"
                />
                ...
                <InputFormField
                    label={fieldConfigs.min_percentage.label}
                    name="min_percentage"
                    inputValue={formData.min_percentage}
                    error={errors.min_percentage}
                    required={fieldConfigs.min_percentage.required}
                    onChange={handleChange}
                    col="col-md-3"
                    type="num"
                />
