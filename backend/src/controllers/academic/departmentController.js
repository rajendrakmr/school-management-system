    ...
    try {
        ...
        if (existing) {
            ...
            return res.status(422).json({
                errors: {
                    code: `Department name "${name}" already exists ${me}`
                }
            });
        }
        ...
        res.status(200).json({ message: `Department "${response.name}" has been successfully created.` });
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        const response = await DepartmentModel.findByPk(id, { transaction: t });
        if (!response) {
            await t.rollback();
        ...
        const existing = await DepartmentModel.findOne({
            where: { name, trn_school_id, mst_department_id: { [Op.ne]: id } },
            transaction: t
        });
        ...
        if (existing) {
            ...
            return res.status(422).json({
                errors: {
                    code: `Depatment name "${name}" already exists ${me}`
                }
            });
        }
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            message: `Department "${session.name}" has been successfully deleted.`
        });
