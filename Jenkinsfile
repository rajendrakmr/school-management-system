pipeline {
  ...
  environment { 
    ...
    BRANCH = 'main'
    FRONTEND_IMAGE_NAME = 'erp-frontend-app'
      ...
      steps {
        script {
          if (params.FRONTEND_IMAGE_TAG == '' || params.BACKEND_IMAGE_TAG == '') {
            error("FRONTEND_IMAGE_TAG and BACKEND_IMAGE_TAG must be provided.")
          }
        }
      }
    }
  ...
  stage('SonarQube: Code Analysis') {
    ...
    stage("Docker: Build Images") {
      steps {
        script {
          env.FRONTEND_IMAGE_TAG_FINAL = params.FRONTEND_IMAGE_TAG ?: "${BUILD_NUMBER}"
          env.BACKEND_IMAGE_TAG_FINAL  = params.BACKEND_IMAGE_TAG  ?: "${BUILD_NUMBER}"
          ...
          dir('backend') {
            sh "docker build -t ${FULL_BACKEND_IMAGE_NAME} ."
          }
        }
      }
    }
    ...
    stage('DockerHub: Push Images') {
      steps {
        withCredentials([usernamePassword(
          ...
          sh "docker push ${FULL_FRONTEND_IMAGE_NAME}"
          sh "docker push ${FULL_BACKEND_IMAGE_NAME}"
        }
      }
    }
  }
